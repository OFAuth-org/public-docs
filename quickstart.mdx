...
<Warning>
  **Use Sandbox for testing!** Before testing with real OnlyFans accounts, use the [Sandbox environment](/getting-started/sandbox) with test credentials. Too many login attempts on production accounts can trigger OnlyFans "suspicious activity" detection, which may cause account resets.
</Warning>
...
<Tip>
  For testing, use a Sandbox API key (`sk_sandbox_...`) with [test credentials](/getting-started/sandbox).
</Tip>
