...
| Get notified when connections change | [Realtime](#realtime) | Recommended |
...
| Store media permanently beyond session expiry | [Vault+](#vault-plus) | Optional |
| Sign requests for direct OnlyFans API calls | [Dynamic Rules](#dynamic-rules) | Advanced |

<Info>
Most integrations use **Link + Access + Realtime**. Media Proxy is automatic (URLs in Access responses are already proxied). Everything else is opt-in.
</Info>
...
<Card title="Realtime Overview" icon="webhook" href="/api-reference/realtime/overview">
...
Persistent media storage that survives session expiry. Unlike Media Proxy (temporary edge caching), Vault+ stores media permanently and serves it by ID.
