...
Read and write OnlyFans data using Connection IDs. Handles request signing, rate limiting, and error recovery automatically.

| Method | Use When |
|---|---|
| **Managed Endpoints** (`/v2/access/*`) | Common operations — stable, typed responses |
| **Proxy Endpoints** (`/v2/access/proxy/*`) | OnlyFans endpoints we haven't wrapped yet |
