...
<Warning>
  **Approval required**: Programmatic Link integrations ("whitelabel" mode) are available only to vetted partners. Contact `support@ofauth.com` with your use case before building against these APIs.
</Warning>
...
<ParamField path="options.mode" type="'whitelabel'" required>
  The session mode. Whitelabel integrations require OFAuth approval.
</ParamField>
