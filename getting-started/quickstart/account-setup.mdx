...
The Media Proxy is **enabled by default**—your API responses will automatically contain `media.ofauth.com/*` URLs instead of raw OnlyFans CDN links. No setup required.

<Warning>
Using these proxied URLs incurs bandwidth charges according to your subscription plan.
</Warning>

- **Per-KB Billing**: You're charged per kilobyte of data transferred—**only on cache miss**
- **Edge Cache**: Content cached globally for up to 7 days (may be purged sooner); cached requests are free
- **CORS Support**: Display images and videos in web apps without cross-origin errors

<Info>
Media proxy is ideal for displaying OnlyFans images and videos in web applications. The proxied URLs work without CORS issues and have extended validity compared to direct OnlyFans CDN links.
</Info>
