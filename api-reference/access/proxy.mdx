...
curl -X GET 'https://api.ofauth.com/v2/access/proxy/users/me' \
  -H 'apikey: YOUR_API_KEY' \
  -H 'x-of-cookie: your_full_session_cookie_string' \
  -H 'x-of-user-agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)...' \
  -H 'x-proxy-url: http://user:password@127.0.0.1:10000'
```
...
<ParamField header="x-proxy-url" type="string" required>
Proxy URL that matches the OnlyFans session in the format: `http://user:pass@host:port` or `socks5://user:pass@host:port`.
</ParamField>
</Tab>
</Tabs>
