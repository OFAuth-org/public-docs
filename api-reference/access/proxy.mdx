...
Provide OnlyFans session data directly if you handle authentication separately. The `x-of-cookie` header must include the OnlyFans `fp`, `auth_id`, and `sess` cookies. A geo-appropriate datacenter proxy is automatically assigned based on your request location.
