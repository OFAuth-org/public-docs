...
Provide OnlyFans session data directly if you handle authentication separately. The `x-of-cookie` header must include the OnlyFans `fp`, `auth_id`, and `sess` cookies.
...
OnlyFans session cookie string. Must contain `fp`, `auth_id`, and `sess`.
</ParamField>
