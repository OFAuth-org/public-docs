...
- The top-level event type is `of.websocket.event.received`
...
2. Route incoming deliveries by the top-level keys present in `data.json`.
...
- `messageType` is transport metadata and is not enough to distinguish event variants on its own.
