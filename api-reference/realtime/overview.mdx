...
<Info>
OnlyFans Events are currently in beta. The top-level event type is stable, and OFAuth now publishes a documented `payloadKey` and `shapeSubtype` for promoted websocket payload families. Additional upstream shapes may still appear over time.
</Info>
