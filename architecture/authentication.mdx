...
All API requests require authentication using your API key and platform ID in the headers:
...
<Note>
	**Platform ID**: The `x-platform-id` header is required for optimal performance. This identifies your platform/organization and enables fast API key validation. You can find your Platform ID in your OFAuth dashboard.
</Note>
...
<Info>
	**Legacy Support**: If you're unable to provide the `x-platform-id` header immediately, the API will still work but with slightly higher latency as it needs to look up your platform from the database. We strongly recommend including this header for best performance.
</Info>
