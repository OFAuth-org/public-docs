...
OFAuth enforces different rate limits based on your subscription plan:

<CardGroup cols={2}>
	<Card title="Pay as You Go" icon="credit-card">
		**30 requests/minute** with burst capacity up to 50 requests
	</Card>
	<Card title="Starter Plan" icon="rocket">
		**60 requests/minute** with burst capacity up to 90 requests
	</Card>
	<Card title="Launch Plan" icon="trending-up">
		**120 requests/minute** with burst capacity up to 200 requests
	</Card>
	<Card title="Scale Plan" icon="chart-line">
		**No OFAuth rate limits** - only subject to OnlyFans' native rate limits
	</Card>
</CardGroup>
...
<Warning>
	**Important**: OFAuth does not bypass OnlyFans' native rate limits. Even if
	...
	responses from OnlyFans depending on their current restrictions.
</Warning>

### Rate Limiting Best Practices

- **Implement exponential backoff** for 429 responses
- **Use multiple connections** for higher throughput when needed
