...
After all parts are uploaded, finalize:
...
<Info>
  For single-part uploads using `PUT /uploads/{mediaUploadId}`, the upload is automatically completed. You don't need to call `/complete` separately.
</Info>
