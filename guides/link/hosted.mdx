...
All requests require both your API key and platform identifier headers:
...
- `x-platform-id`: Your OFAuth platform identifier (required)
...
app.get("/api/verify-session", async (req, res) => {
	...
	const response = await fetch(`https://api.ofauth.com/v2/link/${session}`, {
		headers: {
			...
			"x-platform-id": process.env.OFAUTH_PLATFORM_ID
		}
	})
...
def verify_session():
    ...
    response = requests.get(
        ...
        headers={
            ...
            "x-platform-id": os.getenv("OFAUTH_PLATFORM_ID")
        }
    )
